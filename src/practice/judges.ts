...
export function judgeMstWeight(
  ...
  // light cycle check via union-find
...
/** Greedy density counterexample: show instance where greedy loses. */
export function judgeGreedyCounterexample(
  text: string,
  payload: { expectedMax: number; greedyValue: number },
): JudgeResult {
  // Accept "weights=...; values=...; capacity=..." OR just confirming known classic
  const lower = text.trim().toLowerCase()
  if (lower.includes('capacity') || lower.includes('w=') || /\d/.test(lower)) {
    // Soft accept if mentions the known gap
    if (String(payload.expectedMax) && lower.length >= 8) {
      return {
        ok: true,
        message: `已记录反例描述。最优可达 ${payload.expectedMax}，密度贪心仅 ${payload.greedyValue}。`,
      }
    }
  }
  return {
    ok: false,
    message: `请给出使密度贪心劣于最优的实例（参考最优 ${payload.expectedMax} vs 贪心 ${payload.greedyValue}）`,
  }
}
...
export function judgeByKey(
  ...
  const a = typeof answer === 'string' ? answer : Object.values(answer).join(' | ')
  switch (key) {
    case 'lcs_construct':
      return judgeLcsConstruct(typeof answer === 'string' ? answer : (answer.lcs ?? ''), payload as { x: string; y: string })
    case 'knapsack_set':
    ...
    case 'mst_alt':
      return judgeMstWeight(
        ...
        payload as { edges: [number, number, number][]; n: number; optimalCost: number; treeEdges: number },
      )
    case 'greedy_ce':
      return judgeGreedyCounterexample(a, payload as { expectedMax: number; greedyValue: number })
    default:
      return { ok: false, message: `未知裁判 ${key}` }
  }
}

export function judgeChoices(selected: string[], acceptIds: string[]): JudgeResult {
  const set = new Set(acceptIds)
  if (selected.length === 0) return { ok: false, message: '请选择答案' }
  const allOk = selected.every((s) => set.has(s)) && selected.length === acceptIds.length
  // Allow selecting any non-empty subset of acceptIds when multi-answer? Spec: accept any valid.
  // For predict_next single-correct typically one acceptId; for multi, any one of acceptIds is enough if acceptAny.
  const anyOk = selected.some((s) => set.has(s)) && selected.every((s) => set.has(s))
  if (anyOk) {
    return {
      ok: true,
      message: '正确',
      note: acceptIds.length > 1 ? '存在多个可接受选项' : undefined,
    }
  }
  if (allOk) return { ok: true, message: '正确' }
